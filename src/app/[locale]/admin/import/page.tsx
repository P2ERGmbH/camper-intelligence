    ...
    if (!user || user.role !== 'admin') {
        ...
        return (
            <div className="container mx-auto px-6 py-12 text-center">
                <h1 className="text-3xl font-bold mb-4">{t('access-denied-title')}</h1>
                <p className="text-lg text-muted-foreground">{t('access-denied-message')}</p>
                <Link href={{pathname: '/admin/login'}} className="mt-6 inline-block bg-blue-600 text-white px-6 py-3 rounded hover:bg-blue-700 transition-colors">
                    {t('login-button')}
                </Link>
            </div>
        );
