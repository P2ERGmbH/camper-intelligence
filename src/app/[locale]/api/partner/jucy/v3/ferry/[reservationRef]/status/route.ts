...
export async function GET(request: Request, { params }: { params: { reservationRef: string } }) {
  try {
    ...
    const apiUrl = `${JUCY_BASE_URL}/api/v3/ferry/${reservationRef}/status?accountKey=${JUCY_API_KEY}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch ferry status from Jucy API', details: errorText }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error in Jucy API v3/ferry/{reservationRef}/status:', error);
