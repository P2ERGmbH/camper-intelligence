...
export async function GET(request: Request, context: { params: Promise<{ reservationRef: string, bookingRef: string }> }) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch ferry booking from Jucy API', details: errorText }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error in Jucy API v3/ferry/{reservationRef}/bookings/{bookingRef}:', error);
