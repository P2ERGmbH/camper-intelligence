...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!locationID) {
      return NextResponse.json({ error: 'Missing required parameters for locationdetails' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/locationdetails/${locationID}`;
    ...
    console.error('Error in RCM API locationdetails:', error);
