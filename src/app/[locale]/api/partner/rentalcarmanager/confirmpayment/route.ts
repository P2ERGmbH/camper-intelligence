...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!refno || !data) {
      return NextResponse.json({ error: 'Missing required parameters for confirmpayment' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/confirmpayment/${refno}/?data=${data}`;
    ...
    console.error('Error in RCM API confirmpayment:', error);
