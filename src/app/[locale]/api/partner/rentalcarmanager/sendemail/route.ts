...
export async function GET(request: NextRequest) {
  try {
    ...
    const rcmReservationRef = searchParams.get('rcmReservationRef');

    if (!rcmReservationRef) {
      return NextResponse.json({ error: 'Missing required parameters for sendemail' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/sendemail/${rcmReservationRef}`;
    ...
    console.error('Error in RCM API sendemail:', error);
