...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!refno || !data) {
      return NextResponse.json({ error: 'Missing required parameters for vaultentry' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/vaultentry/${refno}/?data=${data}`;
    ...
    console.error('Error in RCM API vaultentry:', error);
