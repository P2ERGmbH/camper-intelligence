...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!category_type_id || !pickup_location_id || !pickup_location_date || !pickup_location_time || !dropoff_location_id || !dropoff_location_date || !dropoff_location_time || !age_id || !details_flag) {
      return NextResponse.json({ error: 'Missing required parameters for step2' }, { status: 400 });
    }

    let apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/step2/${category_type_id}/${pickup_location_id}/${pickup_location_date}/${pickup_location_time}/${dropoff_location_id}/${dropoff_location_date}/${dropoff_location_time}/${age_id}/${details_flag}`;
    ...
    console.error('Error in RCM API step2:', error);
