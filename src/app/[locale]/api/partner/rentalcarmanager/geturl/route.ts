...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!data) {
      return NextResponse.json({ error: 'Missing required parameters for geturl' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/geturl/?data=${data}`;
    ...
    console.error('Error in RCM API geturl:', error);
