...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!rcm_reservation_reference || !customer_id || !data) {
      return NextResponse.json({ error: 'Missing required parameters for extradriver' }, { status: 400 });
    }

    const apiUrl = `${RCM_BASE_URL}/${RCM_API_KEY}/extradriver/${rcm_reservation_reference}/${customer_id}/?data=${data}`;
    ...
    console.error('Error in RCM API extradriver:', error);
