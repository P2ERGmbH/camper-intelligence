...
export default async function CamperEditPage({ params }: { params: Promise<{ camperId: string, slug: string, locale:string }> }) {
  const errorsTranslations = await getTranslations('errors');
  const  {camperId, slug, locale} = await params;
  ...
  let providerStations: StationWithImageTile[] = [];
